...
void m_recursiveMove(uint8_t depth, uint8_t height, Board *p_board, Move *p_prevMove, uint64_t *p_counter);
...
void __test__moveTree()
{
    ...
    clock_t begin = clock();
    m_recursiveMove(DEPTH, DEPTH, &board, NULL, &posCount);
    clock_t end = clock();
...
void m_recursiveMove(uint8_t depth, uint8_t height, Board *p_board, Move *p_prevMove, uint64_t *p_counter)
{
    ...
    for (uint8_t i = 0; i < p_legalMoves->elements; i++)
    {
        ...
        {
            uint64_t count = 0;
            m_recursiveMove(depth - 1, height, p_board, &p_legalMoves->array[i], &count);
            (*p_counter) += count;
            printf("Count %ld after ", count);
            printMove(&p_legalMoves->array[i]);
        }
        else
        {
            m_recursiveMove(depth - 1, height, p_board, &p_legalMoves->array[i], p_counter);
        }
